...
RAZORPAY_KEY_ID = "rzp_test_SmrikMqKuTovaS"
RAZORPAY_KEY_SECRET = "90BJKYq42jud3eycGReyeKJO"
...
def login():
    ...
    if request.method == 'POST':
        ...
        if email in USERS and USERS[email]['password'] == password:
            ...
            session['paid'] = USERS[email].get('paid', False)
...
def signup():
    ...
    if request.method == 'POST':
        ...
        if not agree:
            ...
            USERS[email] = {'name': name, 'phone': phone, 'password': password, 'cameras': cameras, 'paid': False}
            session['user'] = email
            session['name'] = name
...
def payment():
    ...
    return render_template_string(PAYMENT, amount=total, amount_paise=amount_paise, cameras=cameras, key_id=RAZORPAY_KEY_ID, name=session.get('name'), email=session.get('user'))
